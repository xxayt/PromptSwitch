...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:  # [*, 3, image_size, image_size]
        batch_size = pixel_values.shape[0]
        patch_embeds = self.patch_embedding(pixel_values)  # [*, embed_dim, image_size//patch_size, image_size//patch_size] 
        patch_embeds = patch_embeds.flatten(2).transpose(1, 2)  # [*, num_patches, embed_dim]

        class_embeds = self.class_embedding.expand(batch_size, 1, -1)  # [*, 1, embed_dim]
        embeddings = torch.cat([class_embeds, patch_embeds], dim=1)  # [*, num_patches + 1, embed_dim]
        embeddings = embeddings + self.position_embedding(self.position_ids)
        return embeddings  # [*, num_patches + 1, embed_dim]
...
class CLIPEncoderLayerPT(nn.Module):
    def __init__(self, config: CLIPConfig):
        ...
        self.nf = config.num_frames  # 6
        self.np = config.num_prompts  # 6

    def forward(
        self,
        hidden_states: torch.Tensor,  # [Video embedding; Prompt cube]
        ...
        residual = hidden_states  # [bs * nf, Ls + np, embed_dim]
        hidden_states = self.layer_norm1(hidden_states)

        num_chunks = self.nf // self.np
        hidden_states = hidden_states.reshape(bs, num_chunks, self.np, Ls + self.np, -1)  # [bs, nf//np, np, Ls + np, embed_dim]
        for i in range(num_chunks):
            prompts = hidden_states[:, [i], :, Ls:, :].clone()  # [bs, 1, np, np, embed_dim]
            hidden_states[:, [i], :, Ls:, :] = prompts.transpose(2, 3)  # [bs, 1, np, np, embed_dim]
        hidden_states = hidden_states.flatten(0, 2)  # [bs * nf, Ls + np, embed_dim]
...
class CLIPEncoderPT(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.prompt_embedding = nn.Parameter(torch.randn(self.np, self.np, config.hidden_size))  # 创建模型参数prompt_embedding，大小为[np, np, embed_dim]
    ...
    def forward(
        ...
        prompts = self.prompt_embedding  # [np, np, embed_dim]
        bs = hidden_states.size(0) // self.nf
        num_chunks = self.nf // self.np
        # bs * num_frames, num_prompts, embed_dim
        prompts = prompts[None, None].repeat(bs, num_chunks, 1, 1, 1).flatten(0, 2)  # [bs, nf//np, np, np, embed_dim]
        # bs * num_frames, seq_len + num_prompts, embed_dim
        hidden_states = torch.cat([hidden_states, prompts], dim=1)  # [Video embedding; Prompt cube]: [bs * nf, Ls + np, embed_dim]

        for idx, encoder_layer in enumerate(self.layers):  # encoder_layer: CLIPEncoderLayerPT
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = encoder_layer(
                    hidden_states,  # [Video embedding; Prompt cube]
                    attention_mask,
                    causal_attention_mask,
                    output_attentions=output_attentions,
                )
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.embeddings(pixel_values)  # [*, 3, image_size, image_size] -> [*, num_patches + 1, embed_dim]
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def get_image_features(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,  # pixel_values: [bs, 3, 224, 224]
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> torch.FloatTensor:
...
class PromptCLIP(nn.Module):
    def __init__(self, config: Config):
        ...
        self.config = config
        clip_config = CLIPConfig.from_pretrained("/home/xzj/.cache/huggingface/hub/models--openai--clip-vit-base-patch32/snapshots/e6a30b603a447e251fdaca1c3056b2a16cdfebeb")
        ...
        self.clip = CLIPModel.from_pretrained("/home/xzj/.cache/huggingface/hub/models--openai--clip-vit-base-patch32/snapshots/e6a30b603a447e251fdaca1c3056b2a16cdfebeb", config=clip_config)
        ...
        nn.init.normal_(self.clip.vision_model.encoder.prompt_embedding, mean=0.0, 
            std=clip_config.vision_config.hidden_size**-0.5 * \
                clip_config.vision_config.initializer_range)  # 使用正态分布初始化prompt_embedding参数
    ...
    def forward(self, data, return_all_frames=False):
        ...
        text_data = data['text']
        video_data = data['video']  # [bs, 3, 224, 224]
        video_data = video_data.reshape(-1, 3, self.config.input_res, self.config.input_res)  # [bs, 3, 224, 224]
        ...
        if 'caption' in self.config.loss and self.training:
            ...
            pred_logits = self.forward_captioner(video_features_, text_data['input_ids'][:, :-1])
